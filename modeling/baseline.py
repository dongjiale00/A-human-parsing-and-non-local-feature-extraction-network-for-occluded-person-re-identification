...
from .backbones.mycls_hrnet import HighResolutionNet
from .backbones.mycls_resnet import ResNet, BasicBlock, Bottleneck
from .backbones.mycls_senet import SENet, SEResNetBottleneck, SEBottleneck, SEResNeXtBottleneck
...
class Baseline(nn.Module):
    ...
    def __init__(self, cfg, num_classes, last_stride, model_path, neck, neck_feat, model_name, pretrain_choice):
        ...
        if model_name == 'HRNet32':
            ...
            self.base = ResNet(cfg, last_stride=last_stride,
                               block=Bottleneck,
                               layers=[3, 4, 6, 3]) 
            self.in_planes = 256
        elif model_name == 'se_resnet50':
            self.in_planes = 256
            self.base = SENet(cfg, block=SEResNetBottleneck,
                              layers=[3, 4, 6, 3],
                              groups=1,
                              reduction=16,
                              dropout_p=None,
        ...
        if self.neck == 'bnneck':
            ...
            ## self.classifier_part = nn.Linear(self.in_planes * (self.part_num), self.num_classes, bias=False)
    ...
    def forward(self, x, part_map):
        ...
        y_part, y_global, y_fore, clustering_feat_map, part_pd_score = self.base(x, part_map)  # (b, 2048, 1, 1)
            ...
            if self.neck_feat == 'after':
                ...
                if self.arm:
                    ...
                    for pic in range(part_visible.shape[0]):
                        visible_part.append([int((part_visible[pic]==part).sum()>10) for part in range(1, self.part_num)])
                    ...
                    #return torch.cat((feat_global, feat_fore), 1), clustering_feat_map
