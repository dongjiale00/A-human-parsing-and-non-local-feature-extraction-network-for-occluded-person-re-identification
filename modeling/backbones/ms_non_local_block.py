...
class MSPyramidAttentionContextModule(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, c1, c2, dropout=0, fusion="+", sizes=(1,4,8,16), use_head_bn=False, if_gc=0, vis=False, norm=0):
    ...
    def forward(self, feats, parsing=None, path=None):
        ...
        if self.fusion == "concat":
            ...
            if self.if_gc == 1:
                ...
                if parsing is not None:
                    bottle = parsing + feats + torch.cat(context, 1)
                    print("lal")
                else:
