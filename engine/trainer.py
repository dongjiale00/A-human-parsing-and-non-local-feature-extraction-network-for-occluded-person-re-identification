...
def create_supervised_trainer(model, optimizer,  loss_fn,
    ...
    def _update(engine, batch):
        ...
        img, cls_target, parsing_target = batch
        # img, cls_target, parsing_target, img_2, parsing_target_2 = batch
        # img_2 = img_2.cuda()
        # parsing_target_2 = parsing_target_2.cuda()
        ...
        cls_target = cls_target.cuda()
        parsing_target = parsing_target.cuda()
        cls_score_part, cls_score_global, cls_score_fore, y_part, y_full, y_fore, part_pd_score = model(img, parsing_target)
        loss = loss_fn(cls_score_part, cls_score_global, cls_score_fore, y_part, y_full, y_fore, part_pd_score, cls_target, parsing_target)

        ####

        # cls_score_part_2, cls_score_global_2, cls_score_fore_2, y_part_2, y_full_2, y_fore_2, part_pd_score_2 = model(img_2)
        # loss_2 = loss_fn(cls_score_part_2, cls_score_global_2, cls_score_fore_2, y_part_2, y_full_2, y_fore_2, part_pd_score_2,
        #                cls_target, parsing_target_2)
        # loss = loss + loss_2
...
def create_supervised_evaluator(model, metrics,
    ...
    def _inference(engine, batch):
        model.eval()
        with torch.no_grad():
            data, pids, camids, parsing = batch
            data = data.cuda()
            parsing = parsing.cuda()
            if with_arm:
                g_f_feat, part_feat, part_visible, _ = model(data, parsing)
                return g_f_feat, part_feat, part_visible, pids, camids
            else:
                feat, _ = model(data, parsing)
